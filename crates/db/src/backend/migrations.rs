...
impl CommaEvidence {
    fn observe(&mut self, name: &str) {
        match utils::artist::comma_candidates(name) {
            None => {
                self.solo.insert(utils::artist::name_key(name));
            }
            Some(pieces) => {
                for piece in pieces {
                    self.piece_credits
                        .entry(utils::artist::name_key(piece))
                        .or_default()
    ...
    fn resolve(&self, credit: &str) -> Option<Vec<String>> {
        let pieces = utils::artist::comma_candidates(credit)?;
        let keep: Vec<&str> = pieces
            .iter()
            .copied()
            .filter(|piece| self.attested(&utils::artist::name_key(piece)))
            .collect();
        if keep.is_empty() {
            return None;
        }
        ...
        let confident = keep.len() == pieces.len()
            || keep
                .iter()
                .any(|piece| self.recurs(&utils::artist::name_key(piece)));
